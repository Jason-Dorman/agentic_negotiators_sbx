...
The machine-readable half of `protocol.md` lives in
[`packages/protocol/`](../packages/protocol/README.md): JSON schemas, the EIP-712 digest fixture that
four independent implementations are checked against, the reason-code tables, the ABI artefacts and
the reconstruction tool. That README says which of those files are generated and what regenerating
each one means.
...
| Document | Stage |
|---|---|
| `deployments/<profile>-<date>-<nn>.json` | Stages 1 and 5. Local manifests are git-ignored; Sepolia manifests are committed ([ADR-038](decision_log.md)) |
